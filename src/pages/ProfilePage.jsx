...
import editIcon from '../assets/ProfilePage/editIcon.svg';
import walletIcon from '../assets/ProfilePage/wallet_icon.svg';
...
import redDot from '../assets/ProfilePage/red_dot.svg';
import calIcon from '../assets/ProfilePage/cal_icon.svg';
import addIcon from '../assets/ProfilePage/add_icon.png';
import wishlistIcon from '../assets/ProfilePage/wishlist_icon.svg';
import clockIcon from '../assets/ProfilePage/clock_icon.svg'
...
const ProfilePage = () => {
  ...
  const { toggleWelcomePopup, toggleAddProgramPopup, toggleEditApplicationFormPopup } = usePopup();
  const { customerData } = useAuth();
  // console.log(customerData);
  const navigate = useNavigate();

  const currentPrograms = customerData?.current_programs || {};
  const { programme_id, activity, program_name, program_status } = currentPrograms;
      ...
      <div className={`${utils.container} ${styles.profileContainer}`}>
        <div className={styles.profileMain}>
            ...
            <div className={styles.profileLeftTop}>
              ...
              <div className={styles.profileIcon_type}>
                <div className={styles.profileIconDiv}>
                  ...
                  <p className={`${utils.s12} ${styles.profileType}`}>Customer</p>
                </div>
              </div>
              ...
              <div className={styles.profileSubheading}>
                <p className={utils.s12}>continue your journey and achieve Your Target</p>
              </div>
            </div>
          ...
          <div className={styles.profileRight}>
            <div className={`${programPageStyles.links_session} ${styles.links_session}`}>
              <div className={ProductPageStyles.links}>
                <div className={ProductPageStyles.link_hr}>
                  <p onClick={() => setTabIndex(0)} className={tabIndex === 0 && ProductPageStyles.activeLink}>My Programs</p>
                  {tabIndex === 0 && <span className={`${ProductPageStyles.linkHrLink} ${styles.tabHr}`}></span>}
                </div>
                {/* <div className={ProductPageStyles.link_hr}>
            ...
            <div className={styles.programListDiv}>
              ...
              <div className={styles.programs}>
                ...
                {program_name && <div className={styles.program} onClick={programNavigationHandler}>
                  <p className={styles.programName}>{program_name}</p>
                  <p className={styles.programStatus}>{program_status}</p>
                  {programme_id === 100 ? <div className={`${styles.programTime}`}>
                    <p className={`${utils.s12} ${styles.previousProgramActivity}`}>Join Now</p>
                ...
                <div className={styles.previousPrograms}>
                  {previousPrograms.map(program => <div className={styles.program}>
                    <p className={styles.programName}>{program.program_name}</p>
                    <p className={styles.previousProgramStatus}>{program.program_status}</p>
                    <div className={`${styles.programTime}`}>
                      {/* <p className={`${utils.s12}`}>Next session</p> */}
                      <p className={`${utils.s12} ${styles.previousProgramActivity}`}>{program.activity}</p>
                    </div>
                  </div>)}
                </div>
              </div>
