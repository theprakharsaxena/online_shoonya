...
const ProgramPage = () => {
    ...
    if (customerId && programme_id) {
      try {
        const response = await axios.post(
          `${SERVER_URI}/api/customer/programme_data`,
          {
  ...
  const joinSessionHandler = async () => {
    ...
    if (customerId && programme_id) {
      try {
        const response = await axios.post(
          `${SERVER_URI}/api/customer/join_session`,
          {
            customer_id: customerId,
            programme_id,
          },
  ...
  useEffect(() => {
    ...
    const meetLeaveListener = async () => {
      try {
        console.log("leaved");
        const response = await axios.post(
          `${SERVER_URI}/api/customer/leave_session`,
          {
            customer_id: customerId,
            programme_id,
          },
    ...
    <>
      <div className="py-24 2xl:py-28">
        <div className={landingPageStyles.hero}>
              ...
              <div className={ProductPageStyles.productHeaderTop}>
                <div className={ProductPageStyles.productHeaderNavigation}>
                  <img
                    ...
                    onClick={() => navigate(-1)}
                  />
                  <p className={`${styles.productTitle} ${utils.s32}`}>
                    {programData?.name}
                  </p>
                </div>
          ...
          <div
            className={`${ProductPageStyles.productMain} ${ProductPageStyles.productMainExtra}`}
          >
            <div
              className={`${ProductPageStyles.productContentInfoWrapper} ${styles.productContentInfoWrapperExtra}`}
            >
              <div className={ProductPageStyles.productContentInfo}>
                <div
                  className={`${ProductCardStyles.productPrice} ${ProductPageStyles.productPriceExtra}`}
                >
                  <div className={ProductPageStyles.pricing}>
                    <p className={`${utils.s24} ${styles.programsHeading}`}>
                      Program Timeline
                    </p>
                  </div>
            ...
            <div className={ProductPageStyles.productImgDiv}>
              {!isJoining ? (
                <>
                  ...
                  <div
                    className={`${ProductPageStyles.liveSessionsDiv} ${styles.instructorDiv}`}
                  >
                    <div className={ProductPageStyles.productRatingDiv}>
                      <div
                        className={`${ProductPageStyles.productOwnerProfile} ${styles.instructorProfile}`}
                      >
                        <img src={ProfileImg} alt="product_owner" />
                        <p className={`${styles.instructorName} ${utils.s12}`}>
                          Param Yoga
                        </p>
                      </div>
                    </div>
          ...
          <div className={`${ProductPageStyles.productMain} `}>
            <div className={styles.sessionInfoWrapper}>
              <div
                className={`${ProductPageStyles.linksSection} ${styles.linksSection}`}
              >
                <div className={styles.links_session}>
                  <div className={ProductPageStyles.links}>
                    <div className={ProductPageStyles.link_hr}>
                      ...
                      {tabIndex === 0 && (
                        <span className={ProductPageStyles.linkHrLink}></span>
                      )}
                    </div>
                    <div className={ProductPageStyles.link_hr}>
                      <p
                        ...
                        className={
                          tabIndex === 1 && ProductPageStyles.activeLink
                        }
                      >
                        Community
                      </p>
                  ...
                  <div className={styles.joinSessionDiv}>
                    ...
                    <div className={styles.liveSession}>
                      ...
                      {timeLeft ? (
                        <div
                          className={styles.joinSessionDiv}
                ...
                {tabIndex === 0 && (
                  <div className={ProductPageStyles.courseSection}>
                    <div className={ProductPageStyles.courseBenefits}>
                      <p className={ProductPageStyles.courseBenefitTitle}>
                        Course Description
                      </p>
                      <p className={ProductPageStyles.courseBenefitsDesc}>
                ...
                {tabIndex === 2 && (
                  <div className={ProductPageStyles.reviewSection}>
                    <div className={ProductPageStyles.reviewItem}>
                      <img src={ProfileImg} alt="profile_img" />
                      <div className={ProductPageStyles.reviewInfo}>
                        <p className={ProductPageStyles.reviewPerson}>
                          Leonardo Da Vinci
                        </p>
                        <p className={ProductPageStyles.reviewDesc}>
                          Lorem ipsum, dolor sit amet consectetur adipisicing
                          elit. Voluptatum voluptatibus illo accusantium,
                          officia sapiente recusandae numquam qui mollitia rerum
                          quasi eveniet temporibus, deserunt quo suscipit
                          praesentium est nulla fugit atque.
                        </p>
                      </div>
                    </div>
                    <div className={ProductPageStyles.reviewItem}>
                    ...
                    <div className={ProductPageStyles.reviewBtnDiv}>
                      <button
                        className={`${utils.btn1} ${ProductPageStyles.reviewBtn}`}
                      >
                        Load more reviews
                      </button>
                    </div>
