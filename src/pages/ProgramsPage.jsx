...
const ProgramsPage = () => {
  ...
  const locationList = ['Asia-Pacific', 'Europe', 'North America', 'South America', 'Oceania', 'Africa'];
  const timeList = ['06:00:00', '07:00:00', '08:00:00', '18:00:00', '20:00:00', '22:00:00'];
  const issueList = ['issue 1', 'issue 2', 'issue 3', 'issue 4', 'issue 5', 'issue 6'];
      ...
      <div className={utils.wrapper}>
        <div className={styles.filterSection}>
          <div className={styles.filterSectionLeft}>
            <button className={`${utils.btn1} ${styles.backBtn}`} onClick={() => navigate(-1)}><img src={leftArrow} alt="left_arrow" /><p>Back</p></button>
          </div>
          <div className={styles.filters}>
            <button className={`${utils.btn2} ${styles.allBtn}`}>All</button>
            <Dropdown placeholder='Select the Problem' list={issueList} state={issue} setState={setIssue} />
            <Dropdown placeholder={'Location'} list={locationList} state={location} setState={setLocation} />
            <Dropdown placeholder={'Program Time'} list={timeList} state={time} setState={setTime} />
            <Dropdown placeholder={'Gender'} list={genderList} state={gender} setState={setGender} />
          </div>
