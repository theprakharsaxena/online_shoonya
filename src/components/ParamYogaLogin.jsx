  ...
  const joinSessionHandler = async () => {
    try {
      const response = await axios.post(
        `${SERVER_URI}/api/instructor/join_session`,
        {
  ...
  useEffect(() => {
    ...
    const meetLeaveListener = async () => {
      try {
        console.log("leaved");
        const response = await axios.post(
          `${SERVER_URI}/api/instructor/leave_session`,
          {
            instructor_id: instructorId,
            programme_id: programId,
          },
          ...
          <div className={styles.formSection}>
            ...
            {teacherToken && (
              <div className={styles.labelInput}>
                ...
                <ParamyogaLoginDropdown
                  placeholder={"Select the Program"}
                  list={ProgramsList}
                  state={programName}
                  setState={setProgramName}
                />
            ...
            {!teacherToken && (
              <div className={styles.buttonDiv} onClick={submitHandler}>
                <button className={`${utils.btn2} ${styles.applyBtn}`}>
                  <p>Login</p>{" "}
                </button>
              </div>
