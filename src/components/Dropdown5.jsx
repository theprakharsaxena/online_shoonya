...
const Dropdown5 = ({ placeholder, list, state, setState, width }) => {
  ...
  return (
    <div className={`${DropdownStyles.customDropdown} ${styles.customDropdownExtra} ${DropdownStyles.customDropdown5Styles}`} style={{ width: width }} ref={dropdownRef}>
      <div className={`${DropdownStyles.dropdownHeader} ${Dropdown2Styles.dropdownHeaderExtra}`} onClick={toggleDropdown}>
        <button className={`${utils.btn1} ${Dropdown2Styles.select} ${DropdownStyles.selectExtra}`}><p className={`${Dropdown2Styles.placeholder} ${styles.placeholderExtra}`}>{state ? state : placeholder}</p> <img src={rightArrow} className={DropdownStyles.downArrow} /></button>
      </div>
      {isOpen && (
